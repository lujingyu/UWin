...
+ (WeiboClient *)sharedInstance {
	@synchronized (weiboClient) {
		if (weiboClient == nil) {
			weiboClient = [[self alloc] init];
		}
	}
	return weiboClient;
}
...
- (void)getFollowedTimelineSinceID:(long long)sinceID withMaximumID:(long long)maxID startingAtPage:(int)page count:(int)count {
...
- (void)favorite:(long long)statusId {
    ...
    NSString *postString = [NSString stringWithFormat:@"id=%lld", statusId];
...
- (void)repost:(long long)statusId tweet:(NSString*)tweet {
    ...
    NSString *postString = [NSString stringWithFormat:@"id=%lld&status=%@", statusId, [tweet encodeAsURIComponent]];
...
- (void)comment:(long long)statusId commentId:(long long)commentId comment:(NSString*)comment {
    ...
    NSString *postString;
	if (commentId) {
		postString = [NSString stringWithFormat:@"id=%lld&cid=%lld&comment=%@", statusId, commentId, [comment encodeAsURIComponent]];
	}
	else {
		postString = [NSString stringWithFormat:@"id=%lld&comment=%@", statusId, [comment encodeAsURIComponent]];
	}
...
- (void)sendDirectMessage:(NSString*)text to:(int)recipientedId {
    ...
    NSString *postString = [NSString stringWithFormat:@"text=%@&user_id=%d", [text encodeAsURIComponent], recipientedId];
